        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {categories.map((category) => (
            <div key={category.name} className="group rounded-lg shadow-lg overflow-hidden border border-gray-200 md:px-3">
              <img src={category.image} alt={category.name} className="w-full h-48 object-contain group-hover:scale-105 transition-transform duration-300" />
              <h3 className="text-xl font-bold text-black text-center py-3">{category.name}</h3>
            </div>
          ))}
        </div>
      </div>
